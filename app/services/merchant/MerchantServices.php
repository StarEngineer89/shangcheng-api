...
class MerchantServices extends BaseServices
{
    ...
    {
        $list = $this->getModel()->field('id,mer_name,mer_avatar,mini_banner,type_id')->with(['product'=>function($query){
    ...
    public function setInfo($uid, $mer_info, $address)
    {
        $res = $this->getModel()->where('id', $uid)->update(['mer_info'=>$mer_info, 'mer_address'=>$address]);
        return true;
    }
